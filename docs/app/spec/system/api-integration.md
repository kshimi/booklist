...
| **Gemini API**（ビルド時・AIコメント） | 日替わりサジェスチョン用AIコメント生成 | Node.js スクリプト（手動実行） |
| **Gemini API**（ビルド時・書誌情報充足） | 著者名・ページ数の未設定書籍へのデータ補完 | Node.js スクリプト（手動実行） |
...
`scripts/enrich-gemini.js` から呼び出し、著者名・ページ数が未設定の書籍の情報を取得する。
ブラウザからは呼び出さない（APIキー保護のため）。
...
| エラー | 処理 |
|--------|------|
| 429 (quota exceeded) | 即座に処理中断。直前まで取得した結果を `book-gemini-enrichment.json` に保存して終了 |
| 不正JSONレスポンス | エラーログを出力してスキップ（当該書籍は未充足のまま） |
| その他APIエラー | エラーログを出力してスキップ |

### 出力データ構造: `data/book-gemini-enrichment.json`
