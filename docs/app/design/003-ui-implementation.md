...
> 採用しない場合は素の CSS（`src/index.css` + コンポーネントごとの CSS ファイル）で実装する。
...
| 項目 | 内容 | 決定方法 |
|------|------|---------|
| スタイリング方針 | Tailwind CSS か素の CSS か | ユーザーの選択（本ドキュメントは Tailwind を推奨） |
| GenreChart の実装 | SVG直接描画か recharts 等のライブラリか | 実装フェーズで決定（シンプルな実装を優先） |
| `subgenre` フィルタの状態管理 | `selectedGenre`（大ジャンル）と `selectedSubgenre`（サブジャンル）を独立した状態として持つ | 状態管理設計に従い、2つの状態で管理する |
